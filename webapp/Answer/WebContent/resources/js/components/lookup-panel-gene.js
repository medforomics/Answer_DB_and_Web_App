    ...
    <v-flex :class="getGeneFlexClasses()">
        <v-layout row wrap>
            ...
            <v-flex xs12>
              ...
              <v-card>
                  <v-toolbar dense class="elevation-0" dark color="primary">
                      <div class="title ml-0">Alpha Fold</div>
                      <v-spacer></v-spacer>
                      <v-tooltip bottom>
                          <v-btn icon @click="showAlphaFoldPanel = true" slot="activator">
                              <v-icon>mdi-arrow-expand-vertical</v-icon>
                          </v-btn>
                          <span>Expand All</span>
                      </v-tooltip>
                      ...
                      <v-tooltip bottom>
                          <v-btn icon @click="showAlphaFoldPanel = false" slot="activator">
                              <v-icon>mdi-arrow-collapse-vertical</v-icon>
                          </v-btn>
                          <span>Collapse All</span>
                      </v-tooltip>
                  ...
                  <v-card>
                      <v-card-text class="pa-3">
                      <span class="body-2"><b>Alpha Fold</b>
                        </span>
                        <v-tooltip bottom v-if="uniProtId" v-show="showAlphaFoldPanel">
                            <v-btn icon slot="activator" :href="'https://alphafold.ebi.ac.uk/entry/' + uniProtId"
                                target="_blank" rel="noreferrer">
                                <v-icon class="primary--text">mdi-open-in-new</v-icon>
                            </v-btn>
                            <span>Open {{ lastGene }} in Alpha Fold</span>
                        </v-tooltip>
                      </v-card-text>
                  </v-card>
              </v-card>
  ...
  methods: {
    ...
    geneAjaxSummary(databases) {
      ...
      axios
        ...
        .then((response) => {
          var responseDatabases = response.data.payload
            ? response.data.payload.databases
            : ""
          if (response.data.isAllowed && response.data.success) {
            ...
            for (var i = 0; i < responseDatabases.length; i++) {
              ...
              var summary =
                payload && payload.summary
                  ? formatPubMedLinks(payload.summary)
                  : null
              panelsToOpen.push(summary ? true : false)
              this.geneSummaries[database].summary = summary
              this.geneSummaries[database].url =
                payload && payload.moreInfoUrl ? payload.moreInfoUrl : ""
              this.geneSummaries[database].url2 =
                payload && payload.moreInfoUrl2 ? payload.moreInfoUrl2 : ""
              //    this.geneSummaries[database].loading = false;
            }
            this.ensemblId =
              response.data.payload && response.data.payload.ensembl
                ? response.data.payload.ensembl.ensemblId
                : null
            this.uniProtId =
              response.data.payload && response.data.payload.ensembl
                ? response.data.payload.ensembl.uniProtId
                : null
            this.geneSymbolErrorMessage = this.ensemblId
              ? null
              : "This HUGO symbol doesn't exist."
            ...
            this.handleDialogs(
              response.data,
              this.geneAjaxSummary.bind(null, databases),
            )
          }
        })
        .catch((error) => {
    ...
    getReactomeLocations(levels) {
      ...
      this.reactomeItems = []
      axios
        .get(webAppRoot + "/getReactomeLocations", {
          params: {
            geneTerm: this.currentGene,
            levels: levels,
          },
        ...
        .then((response) => {
          if (response.data.isAllowed && response.data.success) {
            ...
            this.reactomeError =
              this.currentGene + " could not be found in Reactome."
          } else {
            this.handleDialogs(
              response.data,
              this.getReactomeLocations.bind(this, levels),
            )
          }
    ...
    isLoading() {
      return (
        this.geneSummaryLoading || this.reactomeLoading || this.genePlotLoading
      )
    },
    getGeneFlexClasses() {
      if (this.standalone) {
    ...
    fetchGenePlot() {
      if (this.currentGene && this.$refs.plotUtils) {
        ...
        var promise1 = this.$refs.plotUtils.updateBarPlot(
          "/getAlterationByCancer",
          {
            hugoSymbol: this.currentGene,
            plotId: "alterationByCancerPlot" + this.uniqId,
          },
        )
        var promise2 = this.$refs.plotUtils.updateBarPlot(
          "/getCancerbyPercent",
          {
            hugoSymbol: this.currentGene,
        ...
        var promise3 = this.$refs.plotUtils.updateLolliplotPlot(
          "genieLollipopPlot" + this.uniqId,
          "/getGenieGeneLollipop",
          this.currentGene,
        )
    ...
    updateSelectedBarPlot(cancerType) {
      var plotIds = [
        "alterationByCancerPlot" + this.uniqId,
        "cancerByPercentPlot" + this.uniqId,
      ]

Vue.component("variant-details-dialog", {
  props: {
    ...
    annotationCategoriesCNV: {
      default: () => ["Epidemiology", "Prognosis", "Diagnosis", "Therapy"],
      type: Array,
    },
    annotationBreadth: { default: () => ["Chromosomal", "Focal"], type: Array },
    ...
    annotationPhases: {
      default: () => ["Phase 1", "Phase 2", "Phase 3", "Phase 4"],
      type: Array,
    },
    scopesSNP: {
      default: () => ["Case", "Gene", "Variant", "Tumor"],
      type: Array,
    },
    scopesCNV: { default: () => ["Case", "Tumor"], type: Array },
    scopesTranslocation: { default: () => ["Case", "Tumor"], type: Array },
    scopesVirus: {
      default: () => ["Case", "Gene", "Variant", "Tumor"],
      type: Array,
    },
    ...
    somaticItems: {
      default: () => ["Germline", "Somatic", "Unknown"],
      type: Array,
    },
                                            ...
                                            <v-container grid-list-md fluid pl-2 pr-2 pt-2 pb-2>
                                                ...
                                                <v-slide-y-transition>
                                                    ...
                                                    <v-flex xs12 sm12 md6 lg6 xl4 v-for="(annotation, index) in utswAnnotationsFormatted" :key="index" v-show="annotation.visible">
                                                        ...
                                                        :saving-needed="isSaveNeededOverall"
...
`,
  ...
  methods: {
    ...
    isRelatedVariantsVisible() {
      return (
        this.annotationVariantRelatedVisible &&
        this.isSNP() &&
        this.currentVariantHasRelatedVariants
      )
    },
    isRelatedCNVVisible() {
      return (
        this.annotationCNVRelatedVisible &&
        this.isSNP() &&
        this.currentVariantHasRelatedCNV
      )
    },
    ...
    createVariantName() {
      var text =
        this.currentVariant.geneName + " " + this.currentVariant.notation
      if (text.length > 18) {
    ...
    annotationAllHidden() {
      return (
        !this.annotationVariantDetailsVisible &&
    ...
    revertVariant(keepSaveState) {
      if (this.isSNP()) {
        ...
        this.getTranslocationDetails(
          this.$route.query.variantId,
          this.isFirstVariant,
          this.isLastVariant,
        )
      } else if (this.isVirus()) {
        this.getVirusDetails(
          this.$route.query.variantId,
          this.isFirstVariant,
          this.isLastVariant,
        )
    ...
    syncCurrentVariant() {
      var lightVariant = this.$store.getters["variantStore/getLightVariant"](
        this.currentVariant._id["$oid"],
      )
      if (lightVariant) {
        if (this.isSNP()) {
    ...
    getVariantDetails(variantId, isFirstVariant, isLastVariant) {
      return new Promise((resolve, reject) => {
        ...
        axios
          .get(webAppRoot + "/getVariantDetails", {
            params: {
              variantId: variantId,
          ...
          .then((response) => {
            if (response.data.isAllowed) {
              ...
              let value = response.data.patientDetailsOncoTreeDiagnosis
                ? response.data.patientDetailsOncoTreeDiagnosis.value
                : ""
              ...
              var infoTable = {
                ...
                items: [
                  ...
                  },
                  {
                    ...
                    value:
                      this.currentVariant.chrom + ":" + this.currentVariant.pos,
                    type: "link",
                  ...
                  },
                  {
                    ...
                    value: this.currentVariant.numCasesSeen
                      ? this.currentVariant.numCasesSeen + ""
                      : "",
                  },
                  {
              ...
              var oncoKBIds = [
                {
                  ...
                  label:
                    "<span class='no-text-transform'>&nbsp;OncoKB Gene " +
                    this.currentVariant.oncokbGeneName +
                    "&nbsp;</span>",
                },
                {
                  type: "oncoKB",
                  ...
                  label:
                    "<span class='no-text-transform'>&nbsp;OncoKB Variant " +
                    this.currentVariant.oncokbVariantName +
                    "&nbsp;</span>",
                },
              ]
              ...
              for (var i = 0; i < this.currentVariant.ids.length; i++) {
                variousIds.push({
                  ...
                  label: this.formatIdLinkLabel(
                    this.currentVariant.ids[i],
                    this.currentVariant.ids,
                    this.currentVariant.cosmicPatients,
                  ),
                })
              }
                ...
                items: [
                  {
                    ...
                    value: this.currentVariant.tumorTotalDepth
                      ? this.currentVariant.tumorTotalDepth + ""
                      : "",
                  },
                  {
                    label: "Tumor Alt Percent",
                    value: this.currentVariant.tumorAltFrequencyFormatted
                      ? this.currentVariant.tumorAltFrequencyFormatted + "%"
                      : "", //already formatted as pct
                  },
                  {
                    label: "Normal Total Depth",
                    value: this.currentVariant.normalTotalDepth
                      ? this.currentVariant.normalTotalDepth + ""
                      : "",
                  },
                  {
                    label: "Normal Alt Percent",
                    value: this.currentVariant.normalAltFrequencyFormatted
                      ? this.currentVariant.normalAltFrequencyFormatted + "%"
                      : "", //already formatted as pct
                  },
                  {
                    label: "RNA Total Depth",
                    value: this.currentVariant.rnaTotalDepth
                      ? this.currentVariant.rnaTotalDepth + ""
                      : "",
                  },
                  {
                    label: "RNA Alt Percent",
                    value: this.currentVariant.rnaAltFrequencyFormatted
                      ? this.currentVariant.rnaAltFrequencyFormatted + "%"
                      : "", //already formatted as pct
                  },
                  {
                    label: "Exac Allele Frequency",
                    value: this.formatPercent(
                      this.currentVariant.exacAlleleFrequency,
                    ),
                  },
                ],
              }
              if (this.currentVariant.gnomadPopmaxAlleleFrequency != 0) {
                depthTable.items.push({
                  ...
                  value: this.formatPercent(
                    this.currentVariant.gnomadPopmaxAlleleFrequency,
                  ),
                  type: this.currentVariant.gnomadHg19Variant ? "link" : null,
                  linkIcon: "open_in_new",
                  url: this.createGnomadLink(),
                  tooltip:
                    "Open in gnomAD (data may differ from version on gnomAD website)",
                })
              } else {
                depthTable.items.push({
                  label: "gnomAD Pop. Max. Allele Frequency",
              ...
              depthTable.items.push({
                label: "gnomAD HOM",
                value: this.currentVariant.gnomadHomozygotes
                  ? this.currentVariant.gnomadHomozygotes[0] + ""
                  : "",
              })
              ...
              if (response.data.relatedSummary) {
                if (this.$refs.relatedVariantAnnotation)
                  this.$refs.relatedVariantAnnotation.manualDataFiltered(
                    response.data.relatedSummary,
                  )
              ...
              if (response.data.cnvRelatedSummary) {
                if (this.$refs.relatedCNVAnnotation)
                  this.$refs.relatedCNVAnnotation.manualDataFiltered(
                    response.data.cnvRelatedSummary,
                  )
              ...
              if (this.$refs.canonicalVariantAnnotation)
                this.$refs.canonicalVariantAnnotation.manualDataFiltered(
                  response.data.canonicalSummary,
                )
              if (this.$refs.otherVariantAnnotations)
                this.$refs.otherVariantAnnotations.manualDataFiltered(
                  response.data.otherSummary,
                )
              this.userAnnotations =
                this.currentVariant.referenceVariant.utswAnnotations.filter(
                  (a) => a.userId == this.userId,
                )
              this.utswAnnotations =
                this.currentVariant.referenceVariant.utswAnnotations
              // this.reloadPreviousSelectedState();
              this.mdaAnnotations = this.currentVariant.mdaAnnotation
                ? this.currentVariant.mdaAnnotation
                : ""
              ...
              this.handleDialogs(
                response.data,
                this.getVariantDetails.bind(null, variantId),
              )
            }
          })
          .catch((error) => {
        ...
        axios
          ...
          .then((response) => {
            if (response.data.isAllowed) {
              ...
              this.syncCurrentVariant()
              this.currentItem = response.data.item
              this.patientDetailsOncoTreeDiagnosis = {
                text: response.data.patientDetailsOncoTreeDiagnosis.value,
                label: response.data.patientDetailsOncoTreeDiagnosis.text,
              }
              this.addCustomWarningFlagsForItem(this.currentItem)
              this.variantDataTables = []
              var geneChips = []
              ...
              var infoTable = {
                ...
                items: [
                  {
                    label: "Chromosome",
                    value: formatChrom(this.currentVariant.chrom),
                  },
                  {
                    label: "Genes",
                    type: "chip",
                    value: this.currentVariant.geneChips,
                  },
                ],
              }
              var infoTable2 = {
                ...
                items: [
                  {
                    label: "Start",
                    value: this.currentVariant.startFormatted,
                  },
                  {
                    label: "End",
                    value: this.currentVariant.endFormatted,
                  },
                  {
                    label: "Reported Tier",
                    type: "select",
                  ...
                  {
                    label: "Copy Number",
                    value: this.currentVariant.copyNumber
                      ? this.currentVariant.copyNumber + ""
                      : "",
                  },
                  {
                    label: "Score",
                    value: this.currentVariant.score
                      ? this.currentVariant.score + ""
                      : "",
                  },
                  {
                    label: "Cytoband",
                    value: this.currentVariant.cytoband,
                  },
                  {
              ...
              this.userAnnotations =
                this.currentVariant.referenceCnv.utswAnnotations.filter(
                  (a) => a.userId == this.userId,
                )
              this.utswAnnotations =
                this.currentVariant.referenceCnv.utswAnnotations
              this.mdaAnnotations = this.currentVariant.mdaAnnotation
                ? this.currentVariant.mdaAnnotation
                : ""
              // this.reloadPreviousSelectedState();
              ...
              this.handleDialogs(
                response.data,
                this.getCNVDetails.bind(null, variantId),
              )
            }
          })
          .catch((error) => {
    ...
    buildAberrationTypeHelp() {
      var message =
        "amplification: High level copy number gain</br>" +
    ...
    getTranslocationDetails(variantId, isFirstVariant, isLastVariant) {
      return new Promise((resolve, reject) => {
        ...
        axios
          .get(webAppRoot + "/getTranslocationDetails", {
            params: {
              variantId: variantId,
              caseId: this.$route.params.id,
            },
            ...
            if (response.data.isAllowed) {
              ...
              var infoTable = {
                ...
                items: [
                  ...
                  {
                    label: "QC Tags",
                    value: this.currentVariant.filtersFormatted,
                  },
                  {
                    label: "Left Gene",
                    type: "textfield",
                  ...
                  {
                    label: "Left Exons",
                    value: this.currentVariant.leftExons,
                  },
                  {
                    label: "Right Exons",
                    value: this.currentVariant.rightExons,
                  },
                ],
              }
              this.variantDataTables.push(infoTable)
              ...
              var infoTable2 = {
                ...
                items: [
                  {
                    label: "Left Breakpoint",
                    value: this.currentVariant.leftBreakpoint,
                  },
                  {
                    label: "Right Breakpoint",
                    value: this.currentVariant.rightBreakpoint,
                  },
                  {
                    label: "Left Strand",
                    value: this.currentVariant.leftStrand,
                  },
                  {
                    label: "Right Strand",
                    value: this.currentVariant.rightStrand,
                  },
                ],
              }
              this.variantDataTables.push(infoTable2)
              ...
              var infoTable3 = {
                ...
                items: [
                  ...
                  {
                    label: "RNA Reads",
                    value: this.currentVariant.rnaReads
                      ? this.currentVariant.rnaReads + ""
                      : "",
                  },
                  {
                    label: "DNA Reads",
                    value: this.currentVariant.dnaReads
                      ? this.currentVariant.dnaReads + ""
                      : "",
                  },
                  // {
              ...
              this.userAnnotations =
                this.currentVariant.referenceTranslocation.utswAnnotations.filter(
                  (a) => a.userId == this.userId,
                )
              this.utswAnnotations =
                this.currentVariant.referenceTranslocation.utswAnnotations
              this.mdaAnnotations = ""
              // this.reloadPreviousSelectedState();
              ...
              this.handleDialogs(
                response.data,
                this.getTranslocationDetails.bind(null, variantId),
              )
            }
          })
          .catch((error) => {
    ...
    getVirusDetails(variantId, isFirstVariant, isLastVariant) {
      return new Promise((resolve, reject) => {
        ...
        axios
          .get(webAppRoot + "/getVirusDetails", {
            params: {
              variantId: variantId,
              caseId: this.$route.params.id,
            },
          ...
          .then((response) => {
            if (response.data.isAllowed) {
              ...
              if (
                this.currentVariant.SampleId &&
                this.currentVariant.SampleId.indexOf("_N_") > -1
              ) {
                sampleType = "Normal"
              } else if (
                this.currentVariant.SampleId &&
                this.currentVariant.SampleId.indexOf("_T_") > -1
              ) {
                sampleType = "Tumor"
              }
              var infoTable = {
                ...
                items: [
                  {
                    label: "Virus Name",
                    value: this.currentVariant.VirusName,
                  },
                  {
                    label: "Virus Description",
                    value: this.currentVariant.VirusDescription,
                  },
                  {
                    label: "Sample Type",
                    value: sampleType,
              ...
              var infoTable2 = {
                ...
                items: [
                  {
                    label: "Sample ID",
                    value: this.currentVariant.SampleId,
                  },
                  {
                    label: "Accession ID",
                    value: this.currentVariant.VirusAcc,
                  },
                  {
                    label: "Read Count",
                    value: this.currentVariant.VirusReadCount,
                  },
                  {
                    label: "Nb. Cases Seen",
                    value: this.currentVariant.numCasesSeen,
              ...
              this.userAnnotations =
                this.currentVariant.referenceVirus.utswAnnotations.filter(
                  (a) => a.userId == this.userId,
                )
              this.utswAnnotations =
                this.currentVariant.referenceVirus.utswAnnotations
              this.mdaAnnotations = ""
              // this.reloadPreviousSelectedState();
              ...
              this.handleDialogs(
                response.data,
                this.getVirusDetails.bind(null, variantId),
              )
            }
          })
          .catch((error) => {
    ...
    createOncoKBGeniePortalFusion() {
      return (
        oncoKBGeniePortalUrl +
        "Fusion/?gene1=" +
        this.currentVariant.leftGene +
        "&gene2=" +
        this.currentVariant.rightGene +
        "&Oncotree=" +
        this.patientDetailsOncoTreeDiagnosis.text
      )
    },
    ...
    formatAnnotations() {
      ...
      this.utswAnnotationsFormatted = this.formatLocalAnnotations(
        this.utswAnnotations,
        true,
      )
      if (this.mdaAnnotations) {
        this.mdaAnnotationsFormatted = this.formatMDAAnnotations(
          this.mdaAnnotations,
        )
      }
    ...
    formatCNVAnnotations() {
      this.highlightLatestAnnotation = true
      this.utswAnnotationsFormatted = this.formatLocalCNVAnnotations(
        this.utswAnnotations,
        true,
      )
      if (this.mdaAnnotations) {
    ...
    formatTranslocationAnnotations() {
      this.highlightLatestAnnotation = true
      this.utswAnnotationsFormatted = this.formatLocalTranslocationAnnotations(
        this.utswAnnotations,
        true,
      )
      this.matchAnnotationFilter()
    ...
    formatVirusAnnotations() {
      this.highlightLatestAnnotation = true
      this.utswAnnotationsFormatted = this.formatLocalVirusAnnotations(
        this.utswAnnotations,
        true,
      )
      this.matchAnnotationFilter()
    ...
    formatMDAAnnotations(annotations) {
      ...
      for (var i = 0; i < annotations.annotationCategories.length; i++) {
        ...
        var variantSpecific =
          annotations.annotationCategories[i].title == "Functional Annotation"
        var tumorSpecific =
          annotations.annotationCategories[i].title ==
          "Tumor type-specific annotation"
        annotation.geneSpecific = geneSpecific
        annotation.variantSpecific = variantSpecific
        annotation.tumorSpecific = tumorSpecific
        annotation.text = annotations.annotationCategories[i].text
          .replace(/\n/g, "<br/>")
          .replace(
            /(PMID:)([0-9]*)/g,
            `<a href='https://www.ncbi.nlm.nih.gov/pubmed/?term=$2' target="_blank" rel="noreferrer">PMID:$2</a>`,
          )
        ...
        annotation.scopeLevels = [
          ...
          "Variant " + (variantSpecific ? this.currentVariant.notation : ""),
          "Diagnosis",
        ]
        ...
        annotation.scopeTooltip = this.createMDALevelInformation(
          geneSpecific,
          variantSpecific,
          tumorSpecific,
        )
    ...
    getHighlightClass(index) {
      return this.highlightLatestAnnotation &&
        index == 0 &&
        !this.canCopyAnnotation
        ? "highlight-active"
        : ""
    },
        ...
        axios({
          ...
          .then((response) => {
            ...
            if (response.data.isAllowed) {
              ...
              this.handleDialogs(
                response.data,
                this.saveAllVariants.bind(null, response.data.skipSnackBar),
              )
            }
          })
          .catch((error) => {
    ...
    startUserAnnotations() {
      ...
      if (this.isSNP() && this.$refs.annotationDialog) {
        ...
        this.$refs.cnvAnnotationDialog.cnvGeneItems =
          this.currentItem.genes.split(" ")
        this.$refs.cnvAnnotationDialog.startUserAnnotations()
      } else if (
        this.isTranslocation() &&
        this.$refs.translocationAnnotationDialog
      ) {
      ...
      axios({
        ...
        params: {
          ...
          geneId: this.currentVariant.geneName
            ? this.currentVariant.geneName
            : "",
          variantId: this.currentVariant._id.$oid,
        },
        data: {
          annotations: this.userAnnotations,
          annotationIdsForReporting: this.$store.getters[
            "annotationStore/getAnnotationIdsForReporting"
          ](this.currentVariant._id.$oid),
        },
      })
        .then((response) => {
          if (response.data.isAllowed && response.data.success) {
            if (response.data.payload.annotationModified) {
              if (
                response.data.userPrefs &&
                response.data.userPrefs.showGoodies
              ) {
            ...
            this.handleDialogs(
              response.data,
              this.commitAnnotations.bind(null, this.userAnnotations),
            )
    ...
    setSelected() {
      if (
        ...
        this.$store.getters["snpStore/getAllVariantItemsMap"][
          this.urlQuery.variantId
        ]
      ) {
      ...
      if (
        this.isCNV() &&
        this.urlQuery.variantId &&
        this.$store.getters["cnvStore/getAllVariantItemsMap"][
          this.urlQuery.variantId
        ]
      ) {
        this.currentlySelected =
      ...
      if (
        this.isTranslocation() &&
        this.urlQuery.variantId &&
        this.$store.getters["ftlStore/getAllVariantItemsMap"][
          this.urlQuery.variantId
        ]
      ) {
        this.currentlySelected =
      ...
      if (
        this.isVirus() &&
        this.urlQuery.variantId &&
        this.$store.getters["virStore/getAllVariantItemsMap"][
          this.urlQuery.variantId
        ]
      ) {
        this.currentlySelected =
      ...
      return new Promise((resolve, reject) => {
        axios({
          ...
          data: {
            variants:
              this.$store.getters[
                "annotationStore/getAnnotationSelectionToSave"
              ],
          },
        })
          .then((response) => {
            ...
            if (response.data.isAllowed) {
              ...
              this.$store.commit(
                "annotationStore/clearAfterSaving",
                this.urlQuery.variantId,
              )
              resolve({ success: true })
            } else {
              this.handleDialogs(
                response.data,
                this.saveAllAnnotationSelections.bind(
                  null,
                  response.data.skipSnackBar,
                ),
              )
            }
    ...
    handlePanelVisibility(visible) {
      if (visible == null) {
        if (this.urlQuery.edit) {
          this.editAnnotationVariantDetailsVisible =
            !this.editAnnotationVariantDetailsVisible
        } else {
          this.annotationVariantDetailsVisible =
            !this.annotationVariantDetailsVisible
    ...
    matchAnnotationFilter() {
      if (
        ...
        this.searchAnnotationCategory.length > 0 ||
        this.searchAnnotationClassification.length > 0 ||
        this.searchAnnotationTier.length > 0 ||
        ...
        for (var i = 0; i < this.utswAnnotationsFormatted.length; i++) {
          ...
          if (this.searchAnnotationCategory.length > 0) {
            if (
              this.searchAnnotationCategory.includes(
                this.utswAnnotationsFormatted[i].category,
              )
            ) {
          ...
          if (this.searchAnnotationBreadth.length > 0) {
            if (
              this.searchAnnotationBreadth.includes(
                this.utswAnnotationsFormatted[i].breadth,
              )
            ) {
          ...
          if (this.searchAnnotationClassification.length > 0) {
            if (
              this.searchAnnotationClassification.includes(
                this.utswAnnotationsFormatted[i].classification,
              )
            ) {
          ...
          if (this.searchAnnotationTier.length > 0) {
            if (
              this.searchAnnotationTier.includes(
                this.utswAnnotationsFormatted[i].tier,
              )
            ) {
              foundTierMatch = true
            }
          ...
          if (this.searchAnnotationScope.length > 0) {
            for (var j = 0; j < this.searchAnnotationScope.length; j++) {
              ...
              for (
                var k = 0;
                k < this.utswAnnotationsFormatted[i].scopeLevels.length;
                k++
              ) {
                //iterate through scopeLevels to find a match
                if (
                  this.utswAnnotationsFormatted[i].scopeLevels[k].indexOf(
                    scope,
                  ) > -1
                ) {
                  scopeIndex = k
              ...
              if (
                scopeIndex > -1 &&
                this.utswAnnotationsFormatted[i].scopes[scopeIndex]
              ) {
          ...
          this.utswAnnotationsFormatted[i].visible =
            foundTextMatch &&
            foundCategoryMatch &&
    ...
    closeVariantDetails() {
      ...
      if (this.$refs.lookupTool) this.$refs.lookupTool.panelVisible = false
    },
    createUCSCLink() {
      return (
        "https://genome.ucsc.edu/cgi-bin/hgTracks?db=hg38&position=" +
        this.currentVariant.chrom +
        ":" +
    ...
    createGnomadLink() {
      if (this.currentVariant.gnomadHg19Variant) {
        return (
          ...
          this.currentVariant.gnomadHg19Variant
        )
      }
      return ""
    },
    createOldBuildList() {
      ...
      for (var key in this.currentVariant.oldBuilds) {
        builds.push(
          this.currentVariant.oldBuilds[key].chrom +
            ":" +
            this.currentVariant.oldBuilds[key].pos +
            " (" +
            key +
            ")",
        )
    ...
    createOncoKBGeniePortalGene() {
      return (
        oncoKBGeniePortalUrl + "Gene/?gene_name=" + this.currentVariant.geneName
      )
    },
    ...
    formatLocalAnnotations(annotations, showUser) {
      ...
      for (var i = 0; i < annotations.length; i++) {
        ...
        annotation.scopes = [
          annotations[i].isCaseSpecific,
          annotations[i].isGeneSpecific,
          annotations[i].isVariantSpecific,
          annotations[i].isTumorSpecific,
        ]
        annotation.scopeLevels = [
          "Case " +
            (annotations[i].isCaseSpecific ? annotations[i].caseId : ""),
          "Gene " +
            (annotations[i].isGeneSpecific ? annotations[i].geneId : ""),
          "Variant " +
            (annotations[i].isVariantSpecific
              ? this.currentVariant.notation
              : ""),
          "Diagnosis " +
            (annotations[i].isTumorSpecific
              ? annotations[i].oncotreeDiagnosis
              : ""),
        ]
        ...
        annotation.pmids = annotations[i].pmids
        // annotation.nctIds = annotations[i].nctIds;
        annotation.scopeTooltip = this.$refs.annotationDialog
          ? this.$refs.annotationDialog.createLevelInformation(annotations[i])
          : ""
        annotation.tier = annotations[i].tier
        annotation.classification = annotations[i].classification
      ...
      for (var i = 0; i < annotations.length; i++) {
        ...
        annotation.scopeLevels = [
          "Case " +
            (annotations[i].isCaseSpecific ? annotations[i].caseId : ""),
          "Variant " +
            (annotations[i].isVariantSpecific ? this.currentVariant.chrom : ""),
          "Diagnosis",
        ]
        var categoryLabel = annotations[i].category
          ? annotations[i].category
          : ""
        var categoryLabel =
          categoryLabel +
          (annotations[i].breadth ? " " + annotations[i].breadth : "")
        annotation.category = categoryLabel
        annotation.breadth = annotations[i].breadth
        annotation.cnvGenes = annotations[i].cnvGenes
          ? annotations[i].cnvGenes.join(" ")
          : ""
        annotation.createdDate = annotations[i].createdDate
        annotation.createdSince = annotations[i].createdSince
        annotation.modifiedDate = annotations[i].modifiedDate
        ...
        annotation.pmids = annotations[i].pmids
        if (this.$refs.cnvAnnotationDialog)
          annotation.scopeTooltip =
            this.$refs.cnvAnnotationDialog.createLevelInformation(
              annotations[i],
            )
        annotation.tier = annotations[i].tier
        annotation.classification = annotations[i].classification
    ...
    formatLocalTranslocationAnnotations(annotations, showUser) {
      ...
      for (var i = 0; i < annotations.length; i++) {
        ...
        annotation.scopes = [
          ...
          annotations[i].isVariantSpecific,
          annotations[i].isTumorSpecific,
          annotations[i].isLeftSpecific,
          annotations[i].isRightSpecific,
        ]
        annotation.scopeLevels = [
          "Case " +
            (annotations[i].isCaseSpecific ? annotations[i].caseId : ""),
          "Variant " +
            (annotations[i].isVariantSpecific
              ? this.currentVariant.fusionName
              : ""),
          "Diagnosis",
          this.currentVariant.leftGene,
          this.currentVariant.rightGene,
        ...
        if (this.$refs.translocationAnnotationDialog)
          annotation.scopeTooltip =
            this.$refs.translocationAnnotationDialog.createLevelInformation(
              annotations[i],
            )
        annotation.trial = annotations[i].trial
        annotation.warningLevel = annotations[i].warningLevel
    ...
    formatSNPCallers(callers) {
      ...
      var tumorAlleleFrequencys = callers.map((c) =>
        c.tumorAlleleFrequencyFormatted
          ? c.tumorAlleleFrequencyFormatted + "%"
          : null,
      )
      var normalTotalDepths = callers.map((c) => c.normalTotalDepth)
      var normalAlleleFrequencys = callers.map((c) =>
        c.normalAlleleFrequencyFormatted
          ? c.normalAlleleFrequencyFormatted + "%"
          : null,
      )
      var formattedRows = []
    ...
    copyAnnotation(annotation, variantType) {
      ...
      if (newAnnotation.type == "snp") {
        ...
        newAnnotation.cnvGenes =
          annotation.cnvGenes == "" ? [] : annotation.cnvGenes
        ...
        newAnnotation.category = annotation.category
          ? annotation.category.split(" ")[0]
          : null
        newAnnotation.cnvGenes = annotation.cnvGenes
          ? annotation.category.split(" ")
          : []
    ...
    hideAnnotation(annotation) {
      ...
      axios
        .get(webAppRoot + "/hideAnnotations", {
          params: {
            variantId: this.currentVariant._id.$oid,
            caseId: this.$route.params.id,
        ...
        .then((response) => {
          if (response.data.isAllowed && response.data.success) {
            ...
            this.handleDialogs(
              response.data,
              this.hideAnnotation.bind(null, annotation),
            )
          }
        })
        .catch((error) => {
      ...
      axios({
        ...
        .then((response) => {
          if (response.data.isAllowed && response.data.success) {
            ...
            this.handleDialogs(
              response.data,
              this.setDefaultTranscript.bind(null, item),
            )
          }
        })
        .catch((error) => {
    ...
    addCustomWarningFlagsForItem(item) {
      item.iconFlags.iconFlags.forEach((f) => {
        f.chip = false
      })
    ...
    formatChrom(chrom) {
      //needed to call the global function from v-text
    ...
    reloadLookupValues(ref, cnvGeneName, activeButton) {
      ...
      let oncotreeItems = oncotree.filter(
        (o) => o.text == this.patientDetailsOncoTreeDiagnosis.text,
      )
